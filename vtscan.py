...
def get_maketime(filename):
    timeinfo = os.stat(filename)
    return int(timeinfo.st_ctime)
...
def main():
    ...
    # csv文件的初始化
    # header是csv文件各列名
    header = ['apk_name', 'hash', 'harmless', 'malicious', 'suspicious','N-day','engine Num','undetected','detected', 'tag']
    ...
    for file in DetectedFiles:
        ...
        print(file)
        tag = 0
        harmless = resp_json["data"]["attributes"]["last_analysis_stats"]["harmless"]
        malicious = resp_json["data"]["attributes"]["last_analysis_stats"]["malicious"]
        suspicious = resp_json["data"]["attributes"]["last_analysis_stats"]["suspicious"]
        detected = harmless + suspicious + malicious
        undetected = resp_json["data"]["attributes"]["last_analysis_stats"]["undetected"]
        if ( detected > 0):
            tag = 1

        #计算N-day的值
        #文件的检测时间 - 文件的下载时间/24
        Ndays = (resp_json["data"]["attributes"]["last_submission_date"] -get_maketime(file) ) /3600/24
        engine = undetected + detected
        data = [file, get_hash256(file), harmless, malicious, suspicious,int(Ndays),engine,undetected,detected,tag]
        write_data(data)
        print("----------------------检测报告获取完毕---------------------\n")
